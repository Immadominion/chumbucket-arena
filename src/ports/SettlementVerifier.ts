 ...
 */
export class StubSettlementVerifier implements SettlementVerifier {
  private readonly overrides = new Map<MatchId, boolean>();

  constructor(private readonly defaultVerified = true) {}

  setVerified(matchId: MatchId, verified: boolean): void {
    this.overrides.set(matchId, verified);
  }

  async verify(input: SettlementVerificationInput): Promise<SettlementVerificationResult> {
    const verified = this.overrides.get(input.matchId) ?? this.defaultVerified;
    return { verified, detail: verified ? "stub: verified" : "stub: forced unverified" };
  }
}
